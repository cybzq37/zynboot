...
def sys_token():
    ...
    r = requests.post(f"{SYS_BASE}/v1/auth/login",
...
def demo_token():
    ...
    r = requests.post(f"{DEMO_BASE}/v1/sys-proxy/auth/login",
                      json={"username": "root", "password": "Zyn@secure#99"},
                      headers={AUTH_HEADER: ""})
...
class TestAuthDirect:
    ...
    def test_info(self, sys_headers):
        r = requests.get(f"{SYS_BASE}/v1/auth/info", headers=sys_headers)
        data = r.json()
        assert data["code"] == "0", f"info 失败: {data}"
        assert data["data"]["user"]["username"] == "root"

    def test_logout(self, sys_headers):
        r = requests.post(f"{SYS_BASE}/v1/auth/logout", headers=sys_headers)
...
class TestUserDirect:
    ...
    def test_create(self, sys_headers):
        r = requests.post(f"{SYS_BASE}/v1/user", headers=sys_headers,
                          json={"username": "testuser", "password": "Test@1234",
                                "nickname": "测试用户", "status": 1})
        assert r.json()["code"] == "0"

    def test_list(self, sys_headers):
        r = requests.get(f"{SYS_BASE}/v1/user", headers=sys_headers,
    ...
    def test_get_by_id(self, sys_headers):
        ...
        r = requests.get(f"{SYS_BASE}/v1/user/{TestUserDirect.created_id}",
                         headers=sys_headers)
        data = r.json()
        assert data["code"] == "0"
    ...
    def test_update(self, sys_headers):
        ...
        r = requests.put(f"{SYS_BASE}/v1/user/{TestUserDirect.created_id}",
                         headers=sys_headers,
                         json={"username": "testuser", "nickname": "已更新"})
        assert r.json()["code"] == "0"
        # 验证更新
        r2 = requests.get(f"{SYS_BASE}/v1/user/{TestUserDirect.created_id}",
                          headers=sys_headers)
        assert r2.json()["data"]["nickname"] == "已更新"

    def test_delete(self, sys_headers):
        assert TestUserDirect.created_id
        r = requests.delete(f"{SYS_BASE}/v1/user/{TestUserDirect.created_id}",
                            headers=sys_headers)
        assert r.json()["code"] == "0"
...
class TestRoleDirect:
    ...
    def test_create(self, sys_headers):
        r = requests.post(f"{SYS_BASE}/v1/role", headers=sys_headers,
                          json={"code": "test_role", "name": "测试角色", "status": 1})
        assert r.json()["code"] == "0"

    def test_list(self, sys_headers):
        r = requests.get(f"{SYS_BASE}/v1/role", headers=sys_headers)
        data = r.json()
        assert data["code"] == "0"
    ...
    def test_get_by_id(self, sys_headers):
        ...
        r = requests.get(f"{SYS_BASE}/v1/role/{TestRoleDirect.created_id}",
                         headers=sys_headers)
        assert r.json()["data"]["code"] == "test_role"

    def test_update(self, sys_headers):
        assert TestRoleDirect.created_id
        r = requests.put(f"{SYS_BASE}/v1/role/{TestRoleDirect.created_id}",
                         headers=sys_headers, json={"code": "test_role", "name": "已更新角色"})
        assert r.json()["code"] == "0"

    def test_delete(self, sys_headers):
        assert TestRoleDirect.created_id
        r = requests.delete(f"{SYS_BASE}/v1/role/{TestRoleDirect.created_id}",
                            headers=sys_headers)
        assert r.json()["code"] == "0"
...
class TestPermissionDirect:
    ...
    def test_list(self, sys_headers):
        r = requests.get(f"{SYS_BASE}/v1/permission", headers=sys_headers)
        assert r.json()["code"] == "0"

    def test_tree(self, sys_headers):
        r = requests.get(f"{SYS_BASE}/v1/permission/tree", headers=sys_headers)
        data = r.json()
        assert data["code"] == "0"
        assert len(data["data"]) > 0

    def test_create(self, sys_headers):
        r = requests.post(f"{SYS_BASE}/v1/permission", headers=sys_headers,
    ...
    def test_update(self, sys_headers):
        if not TestPermissionDirect.created_id:
            pytest.skip("未创建成功")
        r = requests.put(f"{SYS_BASE}/v1/permission/{TestPermissionDirect.created_id}",
    ...
    def test_delete(self, sys_headers):
        if not TestPermissionDirect.created_id:
            pytest.skip("未创建成功")
        r = requests.delete(f"{SYS_BASE}/v1/permission/{TestPermissionDirect.created_id}",
                            headers=sys_headers)
        assert r.json()["code"] == "0"
...
class TestResourceDirect:
    ...
    def test_list(self, sys_headers):
        r = requests.get(f"{SYS_BASE}/v1/resource", headers=sys_headers)
        assert r.json()["code"] == "0"

    def test_create(self, sys_headers):
        r = requests.post(f"{SYS_BASE}/v1/resource", headers=sys_headers,
    ...
    def test_update(self, sys_headers):
        if not TestResourceDirect.created_id:
            pytest.skip("未创建成功")
        r = requests.put(f"{SYS_BASE}/v1/resource/{TestResourceDirect.created_id}",
    ...
    def test_delete(self, sys_headers):
        if not TestResourceDirect.created_id:
            pytest.skip("未创建成功")
        r = requests.delete(f"{SYS_BASE}/v1/resource/{TestResourceDirect.created_id}",
                            headers=sys_headers)
        assert r.json()["code"] == "0"
...
class TestOrgDirect:
    ...
    def test_list(self, sys_headers):
        r = requests.get(f"{SYS_BASE}/v1/org", headers=sys_headers)
        assert r.json()["code"] == "0"

    def test_tree(self, sys_headers):
        r = requests.get(f"{SYS_BASE}/v1/org/tree", headers=sys_headers)
        assert r.json()["code"] == "0"

    def test_create(self, sys_headers):
        r = requests.post(f"{SYS_BASE}/v1/org", headers=sys_headers,
    ...
    def test_update(self, sys_headers):
        if not TestOrgDirect.created_id:
            pytest.skip("未创建成功")
        r = requests.put(f"{SYS_BASE}/v1/org/{TestOrgDirect.created_id}",
    ...
    def test_delete(self, sys_headers):
        if not TestOrgDirect.created_id:
            pytest.skip("未创建成功")
        r = requests.delete(f"{SYS_BASE}/v1/org/{TestOrgDirect.created_id}",
                            headers=sys_headers)
        assert r.json()["code"] == "0"
...
class TestFileDirect:
    ...
    def test_cleanup(self, sys_headers):
        """清空测试残留文件"""
        r = requests.get(f"{SYS_BASE}/v1/file", headers=sys_headers,
                         params={"bizType": "test", "pageSize": 100})
        if r.status_code == 200 and r.json()["code"] == "0":
            for f in r.json()["data"]["records"]:
                requests.delete(f"{SYS_BASE}/v1/file/{f['id']}", headers=sys_headers)
    ...
    def test_upload(self, sys_headers):
        ...
        r = requests.post(f"{SYS_BASE}/v1/file/upload",
    ...
    def test_get_by_id(self, sys_headers):
        ...
        r = requests.get(f"{SYS_BASE}/v1/file/{TestFileDirect.created_id}",
                         headers=sys_headers)
        data = r.json()
        assert data["code"] == "0"
    ...
    def test_download(self, sys_headers):
        assert TestFileDirect.created_id
        r = requests.get(f"{SYS_BASE}/v1/file/{TestFileDirect.created_id}/download",
    ...
    def test_delete(self, sys_headers):
        assert TestFileDirect.created_id
        r = requests.delete(f"{SYS_BASE}/v1/file/{TestFileDirect.created_id}",
                            headers=sys_headers)
        assert r.json()["code"] == "0"
        # 验证已删除
        r2 = requests.get(f"{SYS_BASE}/v1/file/{TestFileDirect.created_id}",
                          headers=sys_headers)
    ...
    def test_delete_dedup_ref(self, sys_headers):
        ...
        r = requests.delete(f"{SYS_BASE}/v1/file/{TestFileDirect.created_id_2}",
                            headers=sys_headers)
        assert r.json()["code"] == "0"
...
class TestProxyAuth:
    ...
    def test_info(self, demo_headers):
        r = requests.get(f"{DEMO_BASE}/v1/sys-proxy/auth/info", headers=demo_headers)
...
class TestProxyUser:
    def test_page(self, demo_headers):
        r = requests.get(f"{DEMO_BASE}/v1/sys-proxy/user",
    ...
    def test_get_by_id(self, demo_headers):
        ...
        r = requests.get(f"{DEMO_BASE}/v1/sys-proxy/user/{root_id}",
...
class TestProxyRole:
    def test_list(self, demo_headers):
        r = requests.get(f"{DEMO_BASE}/v1/sys-proxy/role", headers=demo_headers)
        data = r.json()
        assert data["code"] == "0"
    ...
    def test_get_by_id(self, demo_headers):
        ...
        r = requests.get(f"{DEMO_BASE}/v1/sys-proxy/role/{role_id}",
                         headers=demo_headers)
...
class TestProxyPermission:
    def test_list(self, demo_headers):
        r = requests.get(f"{DEMO_BASE}/v1/sys-proxy/permission", headers=demo_headers)
        assert r.json()["code"] == "0"

    def test_tree(self, demo_headers):
        r = requests.get(f"{DEMO_BASE}/v1/sys-proxy/permission/tree",
                         headers=demo_headers)
        data = r.json()
        assert data["code"] == "0"
        assert len(data["data"]) > 0
...
class TestProxyResource:
    def test_list(self, demo_headers):
        r = requests.get(f"{DEMO_BASE}/v1/sys-proxy/resource", headers=demo_headers)
        assert r.json()["code"] == "0"


class TestProxyOrg:
    def test_list(self, demo_headers):
        r = requests.get(f"{DEMO_BASE}/v1/sys-proxy/org", headers=demo_headers)
        assert r.json()["code"] == "0"

    def test_tree(self, demo_headers):
        r = requests.get(f"{DEMO_BASE}/v1/sys-proxy/org/tree", headers=demo_headers)
        assert r.json()["code"] == "0"
